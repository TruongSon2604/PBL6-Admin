...
import "../promotion/AddPromotionToStorev2.css";
...
const AddVoucherToStore = ({ url }) => {
  ...
  useEffect(() => {
    if (token) {
      ...
      axios
        .get(`${url}/api/v1/public/voucher/all`)
        .then((response) => {
  ...
  const onSubmitHandler = async (event) => {
    ...
    const payload = {
      ...
      voucherIds: selectedIds, // Ensure selectedIds is an array, if it's not, convert it
    };
    console.log("Payload:", JSON.stringify(payload));
  
    try {
      const response = await axios.post(
        `${url}/api/v1/admin/voucher/apply/store`,
        payload,
        { headers }
      );
      ...
      if (response.data.message) {
        ...
        setTimeout(() => {
          ...
          setSelectedIds([]); // Reset voucher selection
          navigate("/admin/voucher"); // Navigate to the order page
        }, 1500);
      } else {
  ...
  const filteredData2 = dataProduct.filter((product) =>
    product.description.toLowerCase().includes(searchTerm2.toLowerCase())
  );
                ...
                >
                  <td>
                    <input
                      type="checkbox"
                      style={{ color: "black", width: "100%" }}
                      onChange={() => handleCheckboxChange(data.voucherId)}
                      checked={selectedIds.includes(data.voucherId)}
                    />
                  </td>
...
export default AddVoucherToStore;
